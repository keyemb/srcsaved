...
{	
	...
	char* 		R92CFwImageFileName_TSMC ={"RTL8192CE/rtl8192cfwT.bin"};
	char*		R92CFwImageFileName_UMC ={"RTL8192CE/rtl8192cfwU.bin"};
	char*		R8723FwImageFileName_UMC ={"RTL8192CE/rtl8723fw.bin"};
	
	char*		TestChipFwFile = {"RTL8192CE/rtl8192cfw.bin"};
	u8*		FwImage;
	u32		FwImageLen;
	
	char*		pFwImageFileName[1] = {"RTL8192CE/rtl8192cfwT.bin"};
	...
	if(IS_HARDWARE_TYPE_8723(dev))	
	{
		if(IS_VENDOR_8723_A_CUT(priv->card_8192_version))
		{
			pFwImageFileName[0] = R8723FwImageFileName_UMC;
			FwImage = Rtl8192CEFwUMC8723ImgArray;
			FwImageLen = UMC8723ImgArrayLength;
			RT_TRACE(COMP_INIT, " ===> FirmwareDownload91C() fw:Rtl819XFwImageArray_UMC for RTL8723 A CUT\n");
		}
		else
		{
			return RT_STATUS_FAILURE;
	...
	else
	{
		if(IS_NORMAL_CHIP(priv->card_8192_version))
		{
			if(IS_VENDOR_UMC_A_CUT(priv->card_8192_version) && !IS_92C_SERIAL(priv->card_8192_version))
			{
				...
				pFwImageFileName[0] = R92CFwImageFileName_UMC;
				FwImage = Rtl8192CEFwUMCImgArray;
				FwImageLen = UMCImgArrayLength;
				RT_TRACE(COMP_INIT, 
					" ===> FirmwareDownload91C() fw:Rtl819XFwImageArray_UMC\n");

			}
			else
			{
				pFwImageFileName[0] = R92CFwImageFileName_TSMC;
				FwImage = Rtl8192CEFwTSMCImgArray;
...
)
{
	...
	while(!bWriteSucess)
	{
		...
		case 1:
		{
			...
			for(idx= 0; idx < 4; idx++)
			{
				write_nic_byte(dev, BOXReg+idx, BoxContent[idx]);
			}
			break;
		}
		case 2:
		...
		case 4:
		{
			...
			for(idx = 0 ; idx < 2 ; idx ++)
			{
				write_nic_byte(dev, BOXExtReg+idx, BoxExtContent[idx]);
			}		
			for(idx = 0 ; idx < 4 ; idx ++)
			{
				write_nic_byte(dev, BOXReg+idx, BoxContent[idx]);
...
)
{
	...
	if(priv->bFWReady == false)
	{
		RT_TRACE(COMP_INIT , "FillH2CCmd92C(): return H2C cmd because of Fw download fail!!!\n");
		return;
	}
...
void FirmwareSelfReset(struct net_device *dev)
{
		...
		priv->FirmwareSubVersion >= 0x01))
	{
		...
		RT_TRACE(COMP_INIT, "=====> 8051 reset success (%d) .\n", Delay);
	}
}
