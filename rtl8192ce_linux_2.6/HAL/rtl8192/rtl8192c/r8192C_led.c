...
void SwLedOn(	struct net_device *dev , PLED_8190 pLed)
{
	...
	LedCfg = read_nic_byte(dev, REG_LEDCFG2);
	...
	case LED_PIN_LED0:
		write_nic_byte(dev, REG_LEDCFG2, (LedCfg&0xf0)|BIT5|BIT6); 
		break;

	case LED_PIN_LED1:
		write_nic_byte(dev, REG_LEDCFG2, (LedCfg&0x0f)|BIT5); 
		break;
...
void SwLedOff(struct net_device *dev, PLED_8190 pLed)
{
	...
	case LED_PIN_LED0:
		...
		if(priv->bLedOpenDrain == true) 
			write_nic_byte(dev, REG_LEDCFG2, (LedCfg|BIT1|BIT5|BIT6));
		else
			write_nic_byte(dev, REG_LEDCFG2, (LedCfg|BIT3|BIT5|BIT6));
		
		break;

	case LED_PIN_LED1:
		LedCfg &= 0x0f; 
		write_nic_byte(dev, REG_LEDCFG2, (LedCfg|BIT3));
		break;
