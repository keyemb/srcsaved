...
#endif
	PHY_SetBBReg(dev, rFPGA0_TxInfo, 0x3, 0x2);
	PHY_SetBBReg(dev, rFPGA1_TxInfo, 0x300033, 0x200022);


	PHY_SetBBReg(dev, rCCK0_AFESetting, bMaskByte3, 0x45);
	PHY_SetBBReg(dev, rOFDM0_TRxPathEnable, bMaskByte0, 0x23);
	PHY_SetBBReg(dev, rOFDM0_AGCParameter1, 0x30, 0x1);	


	PHY_SetBBReg(dev, 0xe74, 0x0c000000, 0x2);
	PHY_SetBBReg(dev, 0xe78, 0x0c000000, 0x2);
	PHY_SetBBReg(dev, 0xe7c, 0x0c000000, 0x2);
	PHY_SetBBReg(dev, 0xe80, 0x0c000000, 0x2);
	PHY_SetBBReg(dev, 0xe88, 0x0c000000, 0x2);	
}
...
#if 1
	{
		RegVal = read_nic_dword(dev, REG_LEDCFG0);
		write_nic_dword(dev, REG_LEDCFG0, RegVal|BIT23);
	}
#endif
	...
	)
{
	...
	{
		Oldval_0 = (PHY_QueryBBReg(dev, rOFDM0_XATxIQImbalance, bMaskDWord) >> 22) & 0x3FF;
		...
		PHY_SetBBReg(dev, rOFDM0_XATxIQImbalance, 0x3FF, TX0_A);
		PHY_SetBBReg(dev, rOFDM0_ECCAThreshold, BIT(31), ((X* Oldval_0>>7) & 0x1));
		...
		PHY_SetBBReg(dev, rOFDM0_XCTxAFE, 0xF0000000, ((TX0_C&0x3C0)>>6));
		PHY_SetBBReg(dev, rOFDM0_XATxIQImbalance, 0x003F0000, (TX0_C&0x3F));
		PHY_SetBBReg(dev, rOFDM0_ECCAThreshold, BIT(29), ((Y* Oldval_0>>7) & 0x1));
		...
		PHY_SetBBReg(dev, rOFDM0_XARxIQImbalance, 0x3FF, reg);
	
		reg = result[final_candidate][3] & 0x3F;
		PHY_SetBBReg(dev, rOFDM0_XARxIQImbalance, 0xFC00, reg);
	...
	)
{
	...
	{
		Oldval_1 = (PHY_QueryBBReg(dev, rOFDM0_XBTxIQImbalance, bMaskDWord) >> 22) & 0x3FF;
		...
		PHY_SetBBReg(dev, rOFDM0_XBTxIQImbalance, 0x3FF, TX1_A);
		PHY_SetBBReg(dev, rOFDM0_ECCAThreshold, BIT(27), ((X* Oldval_1>>7) & 0x1));
		...
		PHY_SetBBReg(dev, rOFDM0_XDTxAFE, 0xF0000000, ((TX1_C&0x3C0)>>6));
		PHY_SetBBReg(dev, rOFDM0_XBTxIQImbalance, 0x003F0000, (TX1_C&0x3F));
		PHY_SetBBReg(dev, rOFDM0_ECCAThreshold, BIT(25), ((Y* Oldval_1>>7) & 0x1));

		if(bTxOnly)
			return;
		
		reg = result[final_candidate][6];
		PHY_SetBBReg(dev, rOFDM0_XBRxIQImbalance, 0x3FF, reg);
	
		reg = result[final_candidate][7] & 0x3F;
		PHY_SetBBReg(dev, rOFDM0_XBRxIQImbalance, 0xFC00, reg);

		reg = (result[final_candidate][7] >> 6) & 0xF;
		PHY_SetBBReg(dev, rOFDM0_AGCRSSITable, 0x0000F000, reg);
	}
}
	...
	bool		is2T)
{
	...
	if(t==0)
	{
	 	...
	 	_PHY_SaveADDARegisters(dev, ADDA_REG, priv->ADDA_backup,IQK_ADDA_REG_NUM);
		_PHY_SaveMACRegisters(dev, IQK_MAC_REG, priv->IQK_MAC_backup);
		_PHY_SaveADDARegisters(dev, IQK_BB_REG, priv->IQK_BB_backup, IQK_BB_REG_NUM);		
	}
	...
	PHY_SetBBReg(dev, 0x800, BIT24, 0x00);
	...
	PHY_SetBBReg(dev, 0x870, BIT10, 0x01);
	PHY_SetBBReg(dev, 0x870, BIT26, 0x01);	
	PHY_SetBBReg(dev, 0x860, BIT10, 0x00);
	PHY_SetBBReg(dev, 0x864, BIT10, 0x00);
	...
	if(t!=0)
	{
		...
		_PHY_ReloadADDARegisters(dev, ADDA_REG, priv->ADDA_backup,IQK_ADDA_REG_NUM);
	
		_PHY_ReloadMACRegisters(dev, IQK_MAC_REG, priv->IQK_MAC_backup);

	 	_PHY_ReloadADDARegisters(dev, IQK_BB_REG, priv->IQK_BB_backup, IQK_BB_REG_NUM);
		...
		PHY_SetBBReg(dev, 0xe30, bMaskDWord, 0x01008c00);		
		PHY_SetBBReg(dev, 0xe34, bMaskDWord, 0x01008c00);
	...
	)
{
	...
	_PHY_SaveADDARegisters(dev, AFE_REG, AFE_backup,IQK_ADDA_REG_NUM);
	...
	for(path = 0; path < pathbound; path++)
	{
		...
		{				
			for(index = 0; index < APK_BB_REG_NUM ; index++)
			{
				if(index == 0 && isNormal)		
					...
					PHY_SetBBReg(dev, BB_REG[index], bMaskDWord, BB_backup[index]|BIT10|BIT26);
				else
					PHY_SetBBReg(dev, BB_REG[index], BIT10, 0x0);					
			}
		...
		{
			PHY_SetBBReg(dev, 0xe50, bMaskDWord, 0x01008c00);			
			PHY_SetBBReg(dev, 0xe54, bMaskDWord, 0x01008c00);					
		...
		for(index = 0; index < APK_BB_REG_NUM; index++)
		{
			...
			PHY_SetRFReg(dev, (RF90_RADIO_PATH_E)path, 0xc, bMaskDWord, 0x8992e);
			...
			do
			{
				...
				{
					if(path == RF90_PATH_A)
						tmpReg = PHY_QueryBBReg(dev, 0xbd8, 0x03E00000);
					else
						tmpReg = PHY_QueryBBReg(dev, 0xbd8, 0xF8000000);
				}
	...
	_PHY_ReloadADDARegisters(dev, AFE_REG, AFE_backup,IQK_ADDA_REG_NUM);
	...
	)
{
	...
	if(IS_NIC_DOWN(priv))
	{
		PHY_SetBBReg(dev, REG_LEDCFG0, BIT23, 0x01);
	...
	{
		if(bMain)
			PHY_SetBBReg(dev, rFPGA0_XB_RFInterfaceOE, BIT5|BIT6, 0x1);	
		else
			PHY_SetBBReg(dev, rFPGA0_XB_RFInterfaceOE, BIT5|BIT6, 0x2);	
	}
	else
	{
	if(bMain)
	...
	)
{
	...
	if(is2T)
	{
		if(PHY_QueryBBReg(dev, rFPGA0_XB_RFInterfaceOE, BIT5|BIT6) == 0x01)
			return true;
		else 
			return false;
	...
	)
{
	...
	if(bReCovery)
	{
		_PHY_ReloadADDARegisters(dev, IQK_BB_REG, priv->IQK_BB_backup_recover, 10);
		return;		
	...
	_PHY_SaveADDARegisters(dev, IQK_BB_REG, priv->IQK_BB_backup_recover, 10);
...
)
{
	...
	if(DefaultAnt == 0)	
	{
		if(pHalData->AntennaRxPath == ANTENNA_B ||pHalData->AntennaTxPath == ANTENNA_B)	
			rfChg = 1;
	...
	{
		if(pHalData->AntennaRxPath == ANTENNA_A ||pHalData->AntennaTxPath == ANTENNA_A)	
			rfChg = 1;
		pHalData->AntennaRxPath = ANTENNA_B;
		pHalData->AntennaTxPath = ANTENNA_B;
	}
		...
		switch(pHalData->RFChipID)
		{
			case RF_6052:
				...
				if(rfChg)
				{
					...
					{
						if(ulAntennaRx == ANTENNA_A)
							TempVal = 0x2;
						else if(ulAntennaRx == ANTENNA_B)
							TempVal = 0x1;
						else
							return FALSE;
						PHY_SetBBReg(pAdapter, rFPGA0_XA_RFInterfaceOE, 0x300, TempVal);
					}
					else
					{
						TempVal = (r_rx_antenna_ofdm<<4) | (r_rx_antenna_ofdm);
						PHY_SetBBReg(pAdapter, rFPGA1_TxInfo, 0xffffffff, r_ant_select_ofdm_val);		
						PHY_SetBBReg(pAdapter, rFPGA0_TxInfo, 0x0000000f, r_ofdm_tx_en_val);		
						PHY_SetBBReg(pAdapter, rOFDM0_TRxPathEnable, 0x000000ff, TempVal);	
						PHY_SetBBReg(pAdapter, rOFDM1_TRxPathEnable, 0x0000000f, r_rx_antenna_ofdm);	
						PHY_SetBBReg(pAdapter, rCCK0_AFESetting, bMaskByte3, r_ant_sel_cck_val);		
					}
				}
				break;
			default:
